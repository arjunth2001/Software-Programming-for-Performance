...
#pragma omp parallel
    {
        ...
        int acc10, acc11, acc12, acc13, acc14, acc15, acc0, acc1, acc2, acc3, acc4, acc5, acc6, acc7, acc8, acc9; // Accumulators, eliminate data dependencies
                    ...
                    for (aux_i = i; aux_i < aux_limit_i; ++aux_i)
                    {
                        for (aux_j = j; aux_j < aux_limit_j; ++aux_j)
                        {
                            ...
                            for (aux_k = k; aux_k < unroll_limit; aux_k += unroll_factor)
                            {
                                ...
                                acc5 += A[aux_i][aux_k + 5] * B[aux_k + 5][aux_j];
                                acc6 += A[aux_i][aux_k + 6] * B[aux_k + 6][aux_j];
                                acc7 += A[aux_i][aux_k + 7] * B[aux_k + 7][aux_j];
                                acc8 += A[aux_i][aux_k + 8] * B[aux_k + 8][aux_j];
                                acc9 += A[aux_i][aux_k + 9] * B[aux_k + 9][aux_j];
                                acc10 += A[aux_i][aux_k + 10] * B[aux_k + 10][aux_j];
                                acc11 += A[aux_i][aux_k + 11] * B[aux_k + 11][aux_j];
                                acc12 += A[aux_i][aux_k + 12] * B[aux_k + 12][aux_j];
                                acc13 += A[aux_i][aux_k + 13] * B[aux_k + 13][aux_j];
                                acc14 += A[aux_i][aux_k + 14] * B[aux_k + 14][aux_j];
                                acc15 += A[aux_i][aux_k + 15] * B[aux_k + 15][aux_j];
                            }
                            ...
                            // Sum up everything
                            C[aux_i][aux_j] += acc0 + acc1 + acc2 + acc3 + acc4 + acc5 + acc6 + acc7 + acc8 + acc9 + acc10 + acc11 + acc12 + acc13 + acc14 + acc15;
                        }
                    }
                }
            }
